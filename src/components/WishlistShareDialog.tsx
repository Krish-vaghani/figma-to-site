...
const WishlistShareDialog = () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: "My Wishlist",
          text: `Check out my wishlist with ${wishlistCount} items!`,
          url: shareUrl,
        });
      } catch {
        // User cancelled
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="w-64 p-3 rounded-2xl" align="end" sideOffset={8}>
        <p className="text-sm font-semibold mb-3">Share Wishlist</p>
        <div className="space-y-2">
          ...
          >
            {copied ? <Check className="h-4 w-4 text-coral" /> : <Copy className="h-4 w-4" />}
