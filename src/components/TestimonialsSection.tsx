    ...
    >
      <div className="px-4 sm:px-6 pt-5 sm:pt-6 pb-3 sm:pb-4 flex-grow flex flex-col">
        <div className="flex items-center gap-2 mb-2.5 sm:mb-3">
          <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-coral/10 flex items-center justify-center flex-shrink-0">
            <Quote className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-coral fill-coral/30" />
          </div>
          <div className="flex gap-0.5">
            {[...Array(5)].map((_, i) => (
              <svg
                key={i}
                className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${i < stars ? "text-coral fill-coral" : "text-muted-foreground/40"}`}
                viewBox="0 0 20 20"
              >
        ...
        <p
          className="text-foreground/80 text-[11px] sm:text-[13px] leading-relaxed font-normal tracking-wide"
          style={{
            display: "-webkit-box",
            WebkitLineClamp: 3,
      ...
      <div className="border-t border-border/30 mx-4 sm:mx-6" />

      <div className="flex items-center gap-2 sm:gap-2.5 px-4 sm:px-6 py-3 sm:py-3.5">
        <img
          src={imgSrc}
          alt={`${testimonial.name} - Customer testimonial`}
          className="w-8 h-8 sm:w-9 sm:h-9 rounded-full object-cover pointer-events-none ring-2 ring-coral/15"
          loading="lazy"
          draggable={false}
        ...
        <div className="min-w-0">
          ...
          <p className="text-muted-foreground text-[10px] sm:text-xs truncate">{testimonial.title}</p>
        </div>
      </div>
    </div>
  );
